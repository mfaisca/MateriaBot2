package com.materiabot.commands.operaomnia;
...
public class SummonCommand extends _BaseCommand{
	...
	private EmbedBuilder showSummonBoard(Summon summon) throws BotException {
		...
		builder.setAuthor(summon.getName(), "https://dissidiadb.com/summons/" + summon.getName().toLowerCase() + "#summon-board", EmoteUtils.getEmoteClassByName(summon.getName() + "_crystal").getImageUrl());
		builder.setDescription("Click the thumbnail to zoom");
		if(summon.getBoardImage() != null)
			builder.setThumbnail(summon.getBoardImage());
		else
			builder.setThumbnail(EmoteUtils.getEmoteClassByName(summon.getName() + "_crystal").getImageUrl());
		builder.setColor(summon.getElement().getColor());
		builder.addField("Special Boosted Chars", Arrays.asList(summon.getSpecialBoosteds()).stream().map(c -> EmoteUtils.getEmoteText(c.getName())).reduce((s1,  s2) -> s1 + s2).orElse("---"), false);
		Integer[] nodeStats = Arrays.asList(StringUtils.split(summon.getNodeCount(), "|")).stream().map(s -> Integer.parseInt(s)).collect(Collectors.toList()).toArray(new Integer[5]);
		builder.addField("Stats", 
				"**MaxHP+50**: " + MessageUtils.empty(nodeStats[0] >= 10 ? 2 : 4) + "x" + nodeStats[0] + " (" + (50 * nodeStats[0]) + ")" + System.lineSeparator() +
				"**IBrv+15**: " + MessageUtils.empty(nodeStats[1] >= 10 ? 8 : 10) + "x" + nodeStats[1] + " (" + (15 * nodeStats[1]) + ")" + System.lineSeparator() +
				"**MBrv+20**: " + MessageUtils.empty(nodeStats[2] >= 10 ? 4 : 6) + "x" + nodeStats[2] + " (" + (20 * nodeStats[2]) + ")" + System.lineSeparator() +
				"**ATK+8**: " + MessageUtils.empty(nodeStats[3] >= 10 ? 9 : 11) + "x" +  nodeStats[3] + " (" + (8 * nodeStats[3]) + ")" + System.lineSeparator() +
				"**DEF+8**: " + MessageUtils.empty(nodeStats[4] >= 10 ? 10 : 12) + "x" + nodeStats[4] + " (" + (8 * nodeStats[4]) + ")" + System.lineSeparator(), true);
		builder.addField("Points", "Treasures: " + (Arrays.asList("Ifrit", "Brothers").contains(summon.getName()) ? 1700 : (Arrays.asList("Diabolos").contains(summon.getName()) ? 1600 : 1800))  + System.lineSeparator() + "Full Board: 6500", true);
		int i = 0;
		for(SummonPassive sp : summon.getBoardPassives())
			builder.addField(
					sp.getName() + (sp.isRecommended() ? " *(Recommended)*" : ""), 
					i < 3 ? Constants.splitString(sp.getDescription(), MessageUtils.DISCORD_EMBED_MAX_COLUMN_WIDTH).stream().reduce((s1, s2) -> s1 + System.lineSeparator() + s2).orElse("") : sp.getDescription(), 
					i++ < 3);
		return builder;
	}
	private EmbedBuilder showMaterials(Summon summon, String levels) throws BotException {
		int fromLevel = Integer.parseInt(levels.split("-")[0]);
		int toLevel = Integer.parseInt(levels.split("-")[1]);
		if(toLevel <= fromLevel || fromLevel < 1 || toLevel > summon.getMaxLevel())
			throw new BotException("Wrong Format - Levels must be between 1 and the Summon's Max Level, with the first level lower than the second.", 201);
		int[][] materials = summon.getTurns() == 1 || summon.getTurns() == 5 ? specialMaterials : normalMaterials; //Choco/Sylph/Mog
		EmbedBuilder builder = new EmbedBuilder();
		builder.setAuthor(summon.getName(), "https://dissidiadb.com/summons/" + summon.getName().toLowerCase(), EmoteUtils.getEmoteClassByName(summon.getName() + "_crystal").getImageUrl());
		builder.setThumbnail(EmoteUtils.getEmoteClassByName(summon.getName()).getImageUrl());
	...
	private EmbedBuilder showSummon(Summon summon) throws BotException {
		...
		builder.addField(
				(summon.getElement() != null ? summon.getElement().getEmote() : "") + 
				 summon.getAttackName(), summon.getAbility(), false);
		if(summon.getBoardPassives() != null && summon.getBoardPassives().length > 0) {
			String passives = "";
			for(int i = 0; i < summon.getBoardPassives().length; i++) {
				passives += "**" + (summon.getBoardPassives()[i].isRecommended() ? "__" + summon.getBoardPassives()[i].getName() + "__" : summon.getBoardPassives()[i].getName()) + "** - " + summon.getBoardPassives()[i].getShortDesc() + System.lineSeparator();
			}
			builder.addField("Board Passives *(__Recommended__)*", passives, false);
		}
		return builder;
	}
	...
	public void doStuff(final Message event) {
		String message = event.getContentDisplay();
		if(!message.contains(" ")) return;
		String[] text = message.split(" ");
		EmbedBuilder embed = null;
		try {
			Summon s = _Library.JP.getSummon(text[1]);
			if((text.length == 4 && text[2].equals("summon") && text[3].equals("board")) || (text.length == 3 && text[2].equals("board")))
				embed = showSummonBoard(s);
			else if(text.length == 3 && text[2].contains("-")) {
				embed = showMaterials(s, text[2]);
				s = null;
			} else
				embed = showSummon(s);
			Message msg = null;
			...
			if(e.getErrorCode() == 201)
				...
				MessageUtils.sendStatusMessageError(event.getChannel(), "Unexpected Error");
		}
	}
	...
	public String help(final Message event, HelpCommand.HELP_TYPE helpType) {
		...
		if(HelpCommand.HELP_TYPE.SHORT.equals(helpType)){
			...
			ret += "* " + _Listener.COMMAND_PREFIX + "summon <SummonName> [20/30]" + System.lineSeparator();
			ret += ">    Shows info about the summon." + System.lineSeparator();
			ret += ">    The level is optional, defaults to the highest level available." + System.lineSeparator();
			ret += "* " + _Listener.COMMAND_PREFIX + "summon <SummonName> <fromL>-<toL>" + System.lineSeparator();
